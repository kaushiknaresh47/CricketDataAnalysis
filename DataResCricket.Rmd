...
# set to directory with the IPL 2019 files
setwd("/Users/Kaushik/Desktop/ipl_csv/IPL 2019")

# use same path as above
...
# this loop essentially goest through all the files, finds the winner, loser, scores per position for each team, and then outputs the most important usable data which is the winning team's name and their scoring based on position, and the losing team's name and their scoring based on position and the match number (not too important, but used for indexing)
for (i in 1:53){
  ...
  # find the players from both teams
...
# this just shows us which team has how many wins or losses (only in the 2019 season of IPL of course)
wins <- c()
losses <- c()
...
### create empty data frames for each team
...
### make the scores averages per position (1-11)
...
## graphs/ examples of graphs we can use
```{r}
ggplot(data = rcb, aes(1:11, win)) + 
  ...
  labs(color = "Outcome", x = "Batting Position", y = "Runs Scored") + scale_x_continuous(breaks = 1:11, labels = 1:11)
...
For example, the first graph for Royal Challengers Bangalore shows that in 2019 when they won, the batting positions of 2-6 performed better than when they lose. Alternatively, this can be phrased as RCB lost games when the batsmen at positions 2-6 underperformed (of course to say this, we would also need to add an average line regardless of match outcome)

The downward sloping graph towards the end makes sense because in general batsmen that come in after around position 7-8 are bowlers who specialize is bowling rather than batting (although some do both, and are called "All Rounders").
